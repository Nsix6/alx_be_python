...
match priority:
    ...
    case "medium":
        if time_bound == "yes":
            print(f"Note: 'task' is a medium priority task that that requires immediate attention today!")
        else:
            print(f"Note: '{task}' is a medium priority task. Consider completing it when you have free time.")
    case "low":
        if time_bound == "yes":
            print(f"Reminder: '{task}' is a low priority task that that requires immediate attention today!")
        else:
